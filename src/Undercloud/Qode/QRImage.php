...
class QRImage
{
    ...
    private static $availStyle = array(
        QR_STYLE_RECT,
        QR_STYLE_CIRCLE,
        QR_STYLE_ELEGANT,
        QR_STYLE_SPICY
    ...
    private static $availKeyStyle = array(
        QR_KEY_STYLE_RECT,
        QR_KEY_STYLE_CIRCLE,
        QR_KEY_STYLE_ELEGANT,
        QR_KEY_STYLE_SPICY
    );

    private $style = QR_KEY_STYLE_ELEGANT;
    private $keyStyle = QR_STYLE_ELEGANT;
    ...
    private $useCleanCenter = false;
    private $useCenterImage = false;
    ...
    public function randomize()
    {
        ...
        $this
            ->setKeyStyle(self::$availKeyStyle[array_rand(self::$availKeyStyle)])
            ->setStyle(self::$availStyle[array_rand(self::$availStyle)])
            ->setPrimaryColor($getRandomColor())
            ->setBackgroundColor($getRandomColor())
            ->setGradient($getRandomBool())
            ->setRadialGradient($getRandomBool())
            ->setGradientAngle(random_int(0, 360))
            ->setGradientValue(
                $getRandomColor(),
                $getRandomColor()
            );
    ...
    public function setKeyStyle($keyStyle)
    {
        $keyStyle = (int) $keyStyle;
        if (!in_array($keyStyle, self::$availKeyStyle)) {
            $keyStyle = QR_KEY_STYLE_RECT;
        }
    ...
    {
        $style = (int) $style;
        if (!in_array($style, self::$availStyle)) {
            $style = QR_KEY_STYLE_RECT;
        }
    ...
    public function setCleanCenter(bool $flag)
    {
        $this->useCleanCenter = $flag;

        return $this;
    }

    public function setCenterImage($image)
    {
         $this->useCenterImage = $image ? : false;
    ...
    private static function getAnglesKind(array $frame)
    {
        $angles = array();
    ...
    private function resolveSubRectCorners($startX, $startY, $eyeSize)
    {
        $r = $this->pixelPerPoint / 0.75;
        $p = $this->pixelPerPoint;
        $f = $this->outerFrame;
        $fullSize = $eyeSize * $p;

        $drawEyePart = function ($x, $y, $size, $radius, $fill) {
            $s = $size;
            $r = min($radius, $size / 2);

            return sprintf(
                '<path ' .
                'd="M %f %f H %f a %f %f 0 0 1 %f %f V %f a %f %f 0 0 1 %f %f H %f a %f %f 0 0 1 %f %f Z" fill="%s"/>',
                $x,
                $y,
                $x + $s - $r,
                $r,
        ...
        $centerX = $startX + $f + $fullSize / 2;
        $centerY = $startY + $f + $fullSize / 2;
        ...
        return (
            "<g transform='rotate({$angle}, {$centerX}, {$centerY})'>" .
                ...
                $drawEyePart(
                    ...
                    ($eyeSize - 2) * $p,
                    $r * 0.8,
                    $this->useGradient ? '#000' : $this->backgroundColor
                ) .
                $drawEyePart(
                    $startX + 2 * $p + $f,
                    $startY + 2 * $p + $f,
    ...
    private function resolveRectCorners($startX, $startY, $eyeSize)
    {
        ...
        return (
            ...
            '<rect ' .
                ...
                'fill="' . ($this->useGradient ? '#000' : $this->backgroundColor) . '" ' .
    ...
    private function resolveCircleCorners($centerX, $centerY, $eyeSize)
    {
        return (
            ...
            '<circle ' .
                ...
                'stroke="' . ($this->useGradient ? '#000' : $this->backgroundColor) . '" ' .
                'fill="' . ($this->useGradient ? '#000' : $this->backgroundColor) . '" />' .
            '<circle ' .
                'cx="' . $centerX . '" ' .
                'cy="' . $centerY . '" ' .
    ...
    private function resolveKeyCorners()
    {
        ...
        } elseif ($this->keyStyle === QR_KEY_STYLE_SPICY) {
            $subSvg .= $this->resolveSubRectCorners(
                0,
                0,
                $eyeSize
            );
        ...
        foreach ($frame as $line => $columns) {
            foreach ($columns as $column => $cell) {
                ...
                if ($cell == QR_VALUE_FILL) {
                    ...
                    } elseif ($this->style === QR_STYLE_SPICY) {
                        $svg .= (
                            '<rect ' .
                                'x="' . ($realX + 1) . '" ' .
                                'y="' . ($realY + 1) . '" ' .
                                'fill="' . $this->primaryColor . '" ' .
                                'width="' . ($this->pixelPerPoint - 1) . '" ' .
                                'height="' . ($this->pixelPerPoint - 1) . '"/>'
                        );
                    } else {
                        $svg .= (
                            '<rect ' .
    ...
    public function ascii($fill = '██', $empty = '  ', $corners = '██')
    {
        ...
        if ($this->useCleanCenter) {
            list($frame) = $this->resolveCleanCenter($frame);
        }
